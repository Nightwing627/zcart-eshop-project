			    ...
			    <div class="box-header with-border">
			      ...
			      <div class="box-tools pull-right">
					@can('response', $dispute)
						<a href="{{ route('admin.support.dispute.response', $dispute) }}" class="ajax-modal-btn btn btn-default btn-flat">{{ trans('app.response') }}</a>
					@endcan
			      </div>
			    </div> <!-- /.box-header -->
			    <div class="box-body">
			        ...
			        <div class="box-body">
						<div class="form-group">
							...
							@if(Gate::allows('view', $dispute->product))
					            <a href="{{ route('admin.catalog.product.show', $dispute->product_id) }}" class="ajax-modal-btn"><span class="lead indent10">{{ $dispute->product->name }}</span></a>
							@else
	      	...
	      	<div class="box">
		        ...
		        <div class="box-body">
					...
					@if(Gate::allows('view', $dispute->customer))
			            <a href="{{ route('admin.admin.customer.show', $dispute->customer_id) }}" class="ajax-modal-btn small"><span class="lead indent10">{{ $dispute->customer->getName() }}</span></a>
					@else
