		        ...
		        {!! Form::model($system, ['method' => 'PUT', 'route' => ['admin.setting.basic.system.update'], 'files' => true, 'id' => 'form', 'class' => 'form-horizontal', 'data-toggle' => 'validator']) !!}
			        ...
			        <div class="col-sm-3">
				    	@if($can_update)
							<div class="form-group text-center">
								{!! Form::label('maintenance_mode', trans('app.form.maintenance_mode'), ['class' => 'control-label with-help']) !!}
							  	...
							  	<div class="handle">
									<a href="{{ route('admin.setting.system.maintenanceMode.toggle') }}" type="button" class="btn btn-lg btn-secondary btn-toggle {{ $system->maintenance_mode == 1 ? 'active' : '' }}" data-toggle="button" aria-pressed="{{ $system->maintenance_mode == 1 ? 'true' : 'false' }}" autocomplete="off">
						...
						<div class="text-center">
				  		...
				  		@if(isset($system) && Storage::exists(image_path("brands/{$system->id}") . 'medium.png'))
							<div class="form-group text-center">
								<label class="with-help control-label"> {{ trans('app.logo') }}</label>
						    	<img src="{{ get_image_src($system->id, 'brands') }}" width="100%" alt="{{ trans('app.image') }}">
