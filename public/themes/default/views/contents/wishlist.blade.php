        ...
        <div class="col-md-12">
            <div class="product product-list-view">
                ...
                <div class="product-img-wrap">
                    <img class="product-img-primary" src="{{ get_product_img_src($wishlist->product, 'medium') }}" alt="{{ $wishlist->product->name }}" title="{{ $wishlist->product->name }}" />
                    <img class="product-img-alt" src="{{ get_product_img_src($wishlist->product, 'medium', 'alt') }}" alt="{{ $wishlist->product->name }}" title="{{ $wishlist->product->name }}" />
                    <a class="product-link" href="{{ route('show.product', $wishlist->product->slug) }}"></a>
                </div>
                ...
                <div class="product-actions">
                    ...
                    <a class="btn btn-primary flat" href="{{ route('show.product', $wishlist->product->slug) }}">
                        <i class="fa fa-shopping-cart"></i> @lang('theme.button.buy_now')
                    </a>

                    {!! Form::open(['route' => ['wishlist.remove', $wishlist], 'method' => 'delete', 'class' => 'data-form']) !!}
                ...
                <div class="product-info">
                    ...
                    <a href="{{ route('show.product', $wishlist->product->slug) }}" class="product-info-title">{{ $wishlist->product->name }}</a>

                    <div class="product-info-availability">
                        @lang('theme.availability'): <span>@lang('theme.in_stock')</span>
                    </div>
                    <div class="product-info-price">
                        <span class="product-info-price-new">{{ get_formated_currency($wishlist->product->inventories->min('sale_price')) }}</span>
                    </div>
                    <div class="product-info-desc"> {{ $wishlist->product->description }} </div>
...
<div class="row pagenav-wrapper">
    {{ $wishlists->links('layouts.pagination') }}
